	...
	$.fn.datetimepicker = function( opt ) {
		var KEY0 = 48,
			...
			createDateTimePicker = function( input ) {
				var datetimepicker = $('<div id="dateTimePicker"'+(options.id?'id="'+options.id+'"':'')+' '+(options.style?'style="'+options.style+'"':'')+' class="xdsoft_datetimepicker xdsoft_noselect '+options.className+'"></div>'),
                ...
                var DateObject = function(){
					...
					this.setCurrentTime = function( dTime) {
						this.currentTime = (typeof dTime == 'string')? this.strtodatetime(dTime) :  this.isValidDate(dTime) ? dTime:this.now();
						datetimepicker.trigger('xchange.xdsoft');
					};
					...
					this.getCurrentTime = function( dTime) {
					...
					this.nextMonth = function() {
						...
						options.onChangeMonth&&options.onChangeMonth.call&&options.onChangeMonth.call(datetimepicker,this.currentTime,datetimepicker.data('input'));
						datetimepicker.trigger('xchange.xdsoft');
						return month;
					};
					...
					this.strtodatetime = function( sDateTime ) {
						var currentTime = sDateTime?Date.parseDate(sDateTime, options.format): this.now();
					...
					this.strtodate = function( sDate ) {
						var currentTime = sDate?Date.parseDate(sDate, options.formatDate): this.now();
						if( !this.isValidDate(currentTime) )
							currentTime =  this.now();
						return currentTime;
					};
					...
					this.strtotime = function( sTime ) {
						var currentTime = sTime?Date.parseDate(sTime, options.formatTime): this.now();
						if( !this.isValidDate(currentTime) )
							currentTime =  now();
						return currentTime;
					};
				...
				var _xdsoft_datetime = new DateObject();
				mounth_picker
					.find('.xdsoft_today_button')
				...
				calendar
					.on('click.xdsoft','td',function() {
						...
						if( $this.hasClass('xdsoft_disabled'))
							return false;
						_xdsoft_datetime.currentTime.setFullYear( $this.data('year') );
				...
				datetimepicker
					...
					.on('close.xdsoft', function( event ) {
						if( options.validateOnBlur ) {
									if(options.allowBlank && !$.trim(input.val()).length ) {
									input.val("");
								    datetimepicker.data('xdsoft_datetime').empty();
									}else if( !Date.parseDate(input.val(), options.format ) ) {
										input.val((_xdsoft_datetime.now()).dateFormat( options.format ));
