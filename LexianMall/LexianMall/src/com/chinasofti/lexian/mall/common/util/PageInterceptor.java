...
public class PageInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (obj instanceof PageHelper&&CommonUtil.startWithIgnoreCace(boundSql.getSql(), "select")) {
			...
			logger.info("pagation sql execute:"+pageSql);
	...
	private String getMysqlPageSql(PageHelper<?> page, StringBuffer sqlBuffer) {
		int beginPage = (page.getPageNo() - 1) * page.getPageSize();
		sqlBuffer.append(" limit ").append(beginPage).append(",").append(page.getPageSize());
		return sqlBuffer.toString();
	}
